...
void BreakHandler::saveBreakpoints()
{
    ...
    //XXX debuggerCore()->setSessionValue(QLatin1String("Breakpoints"), list);
...
{
    QTC_ASSERT(debuggerCore(), return);
    QVariant value; //XXX = debuggerCore()->sessionValue(QLatin1String("Breakpoints"));
...
QVariant BreakHandler::data(const QModelIndex &mi, int role) const
{
    ...
    switch (role) {
    case Qt::ToolTipRole:
        if (0) //XXX debuggerCore()->boolSetting(UseToolTipsInBreakpointsView))
...
bool BreakHandler::isEngineRunning(BreakpointModelId id) const
{
    if (const DebuggerEngine *e = engine(id)) {
        //XXX const DebuggerState state = e->state();
        //XXX return state != DebuggerFinished && state != DebuggerNotReady;
    }
    return false;
}
...
void BreakHandler::timerEvent(QTimerEvent *event)
{
    ...
    //XXX debuggerCore()->synchronizeBreakpoints();
}
...
void BreakHandler::gotoLocation(BreakpointModelId id) const
{
    ConstIterator it = m_storage.find(id);
    BREAK_ASSERT(it != m_storage.end(), return);
    DebuggerEngine *engine = nullptr; //XXX  debuggerCore()->currentEngine();
    if (it->data.type == BreakpointByAddress) {
        if (engine)
            ; //XXX engine->gotoLocation(it->data.address);
    } else {
        if (engine)
            ; //XXX engine->gotoLocation(
                //XXX Location(it->markerFileName(), it->markerLineNumber(), false));
    }
}
...
{
    if (marker) {
        //XXX marker->setIcon(icon());
        //XXX marker->updateMarker();
    }
}
...
void BreakHandler::BreakpointItem::updateMarker(BreakpointModelId id)
{
    ...
    //XXX if (marker && (file != marker->fileName() || line != marker->lineNumber()))
    //XXX     destroyMarker();

    if (!marker && !file.isEmpty() && line > 0) {
        //XXX marker = new BreakpointMarker(id, file, line);
        //XXX marker->init();
    }
}
...
QString BreakHandler::BreakpointItem::toToolTip() const
{
    ...
    if (engine) {
        //XXX str << "<tr><td>" << tr("Engine:")
        //XXX << "</td><td>" << engine->objectName() << "</td></tr>";
    }
    if (!response.pending) {
